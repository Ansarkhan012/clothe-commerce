...
test("category with an image uses Next Image and object-cover",()=>{assert.match(section,/category\.image_url\s*\?\s*\(\s*<Image[\s\S]*?className="object-cover/);assert.match(section,/sizes="\(max-width: 640px\) 96px, 144px"/)});
...
test("carousel controls derive disabled state from scroll position",()=>{assert.match(section,/disabled=\{!canBack\}/);assert.match(section,/disabled=\{!canForward\}/);assert.match(section,/scrollBy\(\{[\s\S]*?left:[\s\S]*?behavior:\s*"smooth"[\s\S]*?\}\)/)});
