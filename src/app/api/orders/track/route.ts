...
export async function POST(request: Request) {
  if (!consumeRateLimit(`track:${getRequestIp(request)}`, 8, 10 * 60_000)) {
  ...
  try {
    ...
    const { data: items } = await supabase.from("order_items")
      .select("product_name, unit_price, quantity, size, color, image, line_total")
    ...
    return Response.json({ order: { public_order_id: order.public_order_id,
      ...
      created_at: order.created_at, items: items ?? [], history: history ?? [] } });
  } catch {
    return Response.json({ message: "Order not found" }, { status: 404 });
  }
}
