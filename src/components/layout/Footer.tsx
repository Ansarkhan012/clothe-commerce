  ...
  return <footer className="mt-20 bg-brand-green-dark text-white">
    <div className="brand-pattern mx-auto grid max-w-[1440px] gap-12 px-5 py-14 sm:px-8 lg:grid-cols-[1.05fr_2fr] lg:px-12">
      ...
      <div className="grid grid-cols-2 gap-x-7 gap-y-10 sm:grid-cols-4">
        {groups.map((group) => <div key={group.title}><h2 className="mb-5 text-[10px] font-semibold uppercase tracking-[0.18em] text-brand-gold">{group.title}</h2><ul className="space-y-3 text-sm text-white/70">{group.links.map(([label, href]) => <li key={href}><Link href={href} className="transition hover:text-white focus-visible:text-white">{label}</Link></li>)}</ul></div>)}
      </div>
    </div>
